        ...
        for episode in range(self.arglist.episodes):
            ...
            while True:
                ...
                if done or t % self.arglist.update_every == 0:
                    with torch.no_grad():
                        if self.arglist.joint_actor_critic:
                            dist, next_values = self.model(
                                torch.tensor(o_1, dtype=torch.float, device=self.device).unsqueeze(0))
                        else:
                    ...
                    for epoch in range(self.arglist.epochs):
                        ...
                        for i in range(0, O.size(0), self.arglist.batch_size):
                            ...
                            if self.arglist.use_clipped_value_loss:
                                v_pred = values
                                v_pred_clipped = old_values[indices] + \
                                                 torch.clamp(values - old_values[indices], - self.arglist.ppo_clip_term, self.arglist.ppo_clip_term)
                                # Unclipped value
                            ...
                            if self.arglist.joint_actor_critic:
                                ...
                                torch.nn.utils.clip_grad_norm_(self.critic.parameters(),
                                                               self.arglist.gradient_clip_term)
...
def parse_args():
    ...
    parser.add_argument("--joint-actor-critic", action="store_true", default=False,
                        help="joint / separate actor critic")
...
def make_env(env_seed):
    ...
    # from dm_control import suite
    # env = suite.load(domain_name="pendulum", task_name="swingup", task_kwargs={'random': env_seed})
    # env.state_size = 3
    # env.action_size = 1
    # env.simulator = "dm_control"
